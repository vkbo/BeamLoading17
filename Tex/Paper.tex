...
\documentclass[aps,prstab,reprint,amsmath,amssymb,groupedaddress,onecolumn]{revtex4-1}
...
of plasma electrons by the accelerated beam, the electron beam can %PM be accelerated - EA ok
gain large amounts of energy with a narrow final energy spread (\%-level) and without significant emittance growth.
\end{abstract}
...
witness beam optimally loads the wakefield, an increase in absolute energy spread can be kept small. %to a minimum. %PM - EA ok
The concept
...
emittance growth small. %growth to a minimum. PM - EA ok
...
transverse oscillations of the plasma electrons, %forming PM - EA ok
...
radius. This %produces a  PM - EA ok
focusing force %that PM - EA ok
preserves emittance \cite{rosenzweig:1991}.
% EA: I put Rosenzweig original blow out paper as citation here instead of, \cite{lu:2006-1, lu:2006}.
...
as drivers a large number of plasma stages is required to reach %PM - EA ok
...
than the plasma wavelengths needed for such applications, and the beam %will PM - EA ok
does not drive a strong wake \cite{gschwendtner:2016}.  By letting the proton beam undergo
self-modulation before injecting the witness beam into accelerating structure, %most PM - EA ok
stronger wakefields are %formed PM - EA ok
...
bunches along the beam axis with a surrounding halo of defocused particles. This train of bunches resonantly %produces - EA ok
drives
wakefields to large %r PM - EA ok 
amplitudes.
...
increase linearly with radius %PM
...
study emittance preservation.  All simulation results in this paper are %PMcalculated - EA ok
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:PlasmaDenTWake} %PMThe p
...
%PM The baseline AWAKE drive beam current is insufficient to reach the non-linear regime and produce a plasma bubble. - EA ok
...
nominal plasma density at the point where we inject the electron beam \cite{awake_collaboration:2016}.  The plasma electron density depletion and the peak field is replicated using a single %PM proton 
bunch with $1.46\nexp{10}$ protons (or $2.34\unit{nC}$), a %PM bunch
length $\sigma_{z} = 40\unit{\mu m}$ ($7\unit{kA}$), and a transverse size $\sigma_{x,y,proton} = 200\unit{\mu m}$.   The peak density is $0.83\cdot n_{0}$ results in a quasi-linear wake. %PM The beam energy is $400\unit{GeV}$.  
...
%PM not sure what the point of the above sentence is ... - EA ok
In order to prevent large amplitude oscillations %PM
of the witness beam particles, which may cause additional energy spread as well as emittance growth, we %PMstudy
...
%PM Not sure what the point of the above lines is ....
...
%    \sigma_{x,y} =\left(c^2\epsilon_{N}^2m_{e}\varepsilon_{0}n_{pe}e^{2}\gamma \right)^{1/4} \label{EQ:MatchedR} - % PM - EA: not correct
	\sigma_{x,y}=\left(\frac{2c^{2}\varepsilon_{\mathrm{\mathrm{N}}}^{2}m_{e}\varepsilon_{0}}{n_{pe}e^{2}\gamma}\right)^{1/4}  \label{EQ:MatchedR}  % EA 
\end{equation}

We %furthermore PM - EA ok
assume an initial normalized emittance of $\epsilon_{N} = 2\unit{\mu m}$. This emittance is possible to produce with a standard RF-injector, while at the same time yielding a sufficient narrow beam. % EA: comment, perhaps put a ref here [***ref]. 

Beam loading of a short witness beam is sensitive to its position relative to the electric field \cite{tzoufras:2009} %PM
as well as to its dephasing with respect to the wakefields when at low energy.
%PM A low gamma witness beam can easily drift out of phase with the drive beam wakefield. The drift of the beams is proportional to $\gamma^{-2}$,  
To eliminate %initial 
de-phasing of the witness beam, % PM
the initial beam energy %PMwas
is set such
...
% EA: we have studied this to some extent, but unfortunately we do not have a good refence for the last statement, no details in \cite{berglyd_olsen:2015}.

Eq. (\ref{EQ:MatchedR}) yields a transverse size of $\sigma_{x,y}$ of $5.25\unit{\mu m}$, narrow compared to the drive beam $\sigma_{x,y,proton} = 200\unit{\mu m}$.   The bunch length was set to $\sigma_{z}=60\unit{\mu m}$, based on earlier %PM
beam loading studies \cite{berglyd_olsen:2016}.  The charge %PM was
...
% EA: the key figure, figure 2, was not discussed in the text until very far down!  I therefore added (text could possibly be improved)
...
The relatively small size of the witness beam %PM compared to the proton beam put some restrictions on the transverse resolution of the witness beam as we need a simulation box wide enough to contain the larger beam while resolving the narrower one (see figure \ref{Fig:PlasmaDenTWake}). 
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamLoading} Top plot%PM plot shows the - EA ok
: unloaded longitudinal electric field with no witness beam
        (dashed blue line) and %PMthe 
        loaded field (whole blue line) along the beam axis. The beam density along the axis
        for both beams are shown in red. %PM The bottom plot compares the 
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamPS} %PMThe p
...
\begin{figure*}[hbt]
    ...
    \caption{\label{Fig:BeamFilament} Plots \textbf{a} to \textbf{f} show the transverse phase space of the offset
        ...
        \textbf{g} and \textbf{h} represent particles %PM
        not in the ion column (see Fig.~\ref{Fig:BeamEmitt}), with position $1.40\unit{\mu m} < \xi < 1.42\unit{\mu m}$. Plots
        \textbf{d}, \textbf{e} and \textbf{f}, and the red lines in plots \textbf{g} and \textbf{h} represent particles %PM
        in the ion column 
...
Figure \ref{Fig:BeamLoading} shows the results of QuickPIC simulations of the initial time step, for the base case parameters. The $E_{z}$-field
generated by the proton drive beam is seen as the blue line, shown with and without the
electron beam present. With a proton beam density $n_{pb} \simeq n_{0}$, %PMwe 
the wakefields are in the quasi-linear regime
...
The witness beam generates its own wakefield which %PM partially cancels out 
loads the $E_{z}$ field generated by the drive beam.
With an ideally shaped electron beam charge profile it is possible to optimally load the field in such a way that the
accelerating field is constant along the beam \cite{katsouleas:1987, tzoufras:2009}.  Gaussian beams, as assumed in these studies, cannot
completely flatten the electric field in the tails of the charge distribution, and our base case beam therefore has a
tail in energy both at the front and the back of the beam, as illustrated in figure \ref{Fig:BeamPS}.  The bulk of
the beam, however, sees a relatively flat field.  
...
in the formation of an ion column %PM 
(Fig.~\ref{Fig:BeamLoading} bottom). This ion column, as is well known \cite{rosenzweig:1991}, provides a linear focusing
force on the part the electron beam within the column, and therefore prevents emittance growth for this %PMregion
part of the
...
Emittance growth mainly occurs in first few meters, and no significant emittance growth %PMwas
...
around the axis of the drive beam wakefield.% EA: would be good to include a statement of how large the offsets can be before when the electron beam starts to break up / not preserve emittance.

The transverse beam size within the bubble, where normalized emittance is preserved, follows the evolution given by
equation \ref{EQ:MatchedR} %PM where the plasma density is given by the ion density - EA ok
...
no significant overloading %PMwas
is observed.  
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamQ} Ratio of total beam charge with an emittance growth $\Delta\epsilon \leq 5\%$ as a
        function of initial beam charge (blue %PM
        symbols, lines), and relative energy spread of the accepted charge (red %PM
        symbols, lines), after
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamQAbs} Total beam charge with an emittance growth $\Delta\epsilon \leq 5\%$ as a function of
        initial beam charge (blue %PM
        symbols, lines), and final %PMmomentum
        energy (red %PM
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamQEmit} Ratio of total beam charge with an emittance growth $\Delta\epsilon \leq 5\%$ as a
        function of initial emittance (blue %PM
        symbols, lines), and relative energy spread of the accepted charge (red %PM
...
The beam loading and blow out properties of the electron beam depend on a large number of parameters, including %PMthe
longitudinal profile, %PMthe 
transverse profile as well as %PMthe 
...
Figure \ref{Fig:BeamQ}  show the dependence of %PM emittance
charge and energy spread on bunch length and %PMbunch
incoming charge.  An initial beam
emittance of $2\unit{\mu m}$ was used.  %
%PM
Therefore, we define the fractional charge as that whose emittance remains smaller than $2.1\unit{\mu m}$. %PM We ran the scan with beam charges from 
The beam charge range $10\unit{pC}$ to $300\unit{pC}$ and %with length
the $\sigma_{z}$ range from $40\unit{\mu m}$ to $100\unit{\mu m}$.  As can be seen from figure \ref{Fig:BeamQ} %, red curves
(red curves), both the
$40\unit{\mu m}$ and the $60\unit{\mu m}$ beam %PMhas
...
of the field, $\approx\lambda_{pe}/4$, %PMwill
does not optimally load the wake, thus producing a larger spread in energy.  The blue curves show the fraction of %PMthe sliced emittance where the
charge whose initial emittance is preserved (the slice%Md
...
%PM SHOULD WE ADD SOMETHING LIKE:
...
Figure \ref{Fig:BeamQAbs} shows the dependence of mean energy gain on bunch length and bunch charge %PM, in red curves
...
emittance.  In these simulations %PMthe focusing of the beam to the plasma remains matched for each emittance value.
...
%We have studied a new plasma wakefield acceleration regime; acceleration of a strongly loaded electron beam in a quasi-linear proton driven wake. When optimally loaded an electron witness beam has sufficient charge to create a strong blow out forming a %n ion 
%bubble trailing the witness beam.  The focusing forces from the ions ensures emittance preservation for large parts of the %PM 
%witness beam. %PM, also when the electron beam has some offset with respect to the proton driver.
%Simulations show that the emittance of a large fraction of the electron beam is preserved even when it enters the plasma with and offset with respect to the proton bunch axis. %

%Using AWAKE parameters these result indicates that emittance preservation for an electron beam injected on-axis into a plasma stage, and properly optimized, may be
%feasible. Parameter studies indicate that up to a few $100\unit{pC}$ might be accelerated, for bunches of length
%$40-60\unit{\mu m}$. Such electron bunches may be generated by an injector based on a standard RF gun [S. Doebert, private communication].

%This study assumes a simplified case with respect to a self-modulated proton beam, where the wake is driven by a single, short proton bunch, and investigates an ideal, uniform plasma profile.  To fully investigate optimal AWAKE injection, further study should be performed including simulations of realistic plasma profiles combined with a self-modulated proton beam.  
% EA: we could also say "is underway" and cite Lotov et co?
%PM: NOT SURE WHAT PLASMA PROFILE WEA RE TALKING ABOUT ... I would suggest - EA patric's verson of conclusions are OK with me
...
% EA: not needed (checked with Jorge)
...
% EA: instead I suggest something like (also promised Jorge)
