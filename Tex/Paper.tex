...
density is matched to the SPS proton beam such that $k_{pe}\sigma_{r} = 1$ where $k_{pe} = 2\pi/\lambda_{pe}$ is the
...
$\sigma_{r} = 200\unit{\mu m}$. The length of the bunch $\sigma_{z} = 40\unit{\mu m}$.
...
A higher charge beam will generate a non-linear wakefield sooner, which in turns ensures that a smaller portion of the
beam experiences the quasi-linear region with emittance growth. As figure \ref{Fig:BeamQAbs} illustrates, a higher
charge also results in a lower energy gain.
