...
of plasma electrons by the accelerated beam, the electron beam can %PM be accelerated
...
witness beam optimally loads the wakefield, an increase in absolute energy spread can be kept small. %to a minimum. %PM
The concept
...
emittance small. %growth to a minimum. PM
...
transverse oscillations of the plasma electrons, %forming PM
...
radius. This %produces a  PM
focusing force %that PM
...
as drivers a large number of plasma stages is required to reach %PM
...
than the plasma wavelengths needed for such applications, and the beam %will PM
does not drive a strong wake \cite{gschwendtner:2016}.  By letting the proton beam undergo
self-modulation before injecting the witness beam into accelerating structure, %most PM
stronger wakefields are %formed PM
...
here we use a single, non-evolving proton bunch as driver.  The proton bunch parameters are chosen so that key features of the wake - the plasma electron density in the wake and the longitudinal electric field - are the same as %PM
in the wake of a self-modulated proton beam with AWAKE baseline parameters \cite{gschwendtner:2016}.  Both the proton beam and the witness have Gaussian %PM
...
study emittance preservation.  All simulation results in this paper are %PMcalculated
...
%PM not sure what the point of the above sentence is ...
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BeamLoading} The top%PM plot shows the 
...
around the axis of the drive beam wakefield, as can bee seen %PMfrom 
in figures \ref{Fig:BeamFilament}%PMg-h.
g and h.

The transverse beam size within the bubble, where normalized emittance is preserved, follows the evolution given by
equation \ref{EQ:MatchedR}%PM where the plasma density is given by the ion density
...
We note here that operation with 100\unit{pC}$ leads to a significantly larger charge (factor $\sim4$) with emittance preserved, at the expense of an increase of relative energy spread by a factor of two. %???
